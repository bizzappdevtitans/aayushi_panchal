...
class ClassDetails(models.Model):
    ...
    courses = fields.One2many("course.details", "course_class", "Course :")
    ...
    def unlink(self):
        if self.class_faculty or self.course:
            raise UserError(_("You cannot Delete this class"))
    ...
    def name_get(self):
        result = []
        for rec in self:
            result.append((rec.id, "%s - %s" % (rec.room_number, rec.class_name)))
        return result
